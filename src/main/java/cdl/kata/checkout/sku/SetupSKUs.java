...
public class SetupSKUs {
	...
	public Map<String, SKU> addSKUs() {
		...
		while (enterSKU) {
			...
			try {
				...
				if (validUserInputs(item, price, quantity, specialPrice)) {
	...
	private boolean validUserInputs(String item, BigDecimal price, int quantity, BigDecimal specialPrice) {
		return item != null && price != null && quantity > 0 && specialPrice != null;
	}
